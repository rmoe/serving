...
func (c *Reconciler) tls(ctx context.Context, host string, r *v1alpha1.Route, traffic *traffic.Config, clusterLocalServiceNames sets.String) ([]netv1alpha1.IngressTLS, error) {
	...
	for _, desiredCert := range desiredCerts {
		...
		cert := findMatchingWildcardCert(desiredCert.Spec.DNSNames, allWildcardCerts)
...
func findMatchingWildcardCert(domains []string, certs []*netv1alpha1.Certificate) *netv1alpha1.Certificate {
	for _, cert := range certs {
		if wildcardCertMatches(domains, cert) {
			return cert
		}
	}
	return nil
...
func wildcardCertMatches(domains []string, cert *netv1alpha1.Certificate) bool {
	dnsNames := sets.NewString()

	for _, dns := range cert.Spec.DNSNames {
		dnsNames.Insert(strings.SplitAfterN(dns, ".", 2)[1])
	}
	for _, domain := range domains {
		testDomain := strings.SplitAfterN(domain, ".", 2)[1]
		if !dnsNames.Has(testDomain) {
			return false
		}
	}
